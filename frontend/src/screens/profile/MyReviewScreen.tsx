...
export default function MyReviewScreen() {
  ...
  const fetchMyReviews = useCallback(async () => {
    ...
    try {
      const myReviews = await getBookReviews(undefined, { mine: true });
      setReviews(myReviews);
    } catch (error) {
      setReviews([]);
      setLoadError('내 리뷰를 불러오지 못했습니다.');
    } finally {
      setIsLoading(false);
    }
  }, []);
          ...
          <View style={styles.emptyWrap}>
            {isLoading ? (
              <>
                ...
                <Text style={styles.emptyText}>내 리뷰를 불러오는 중이에요.</Text>
              </>
            ) : loadError ? (
              ...
              <Text style={styles.emptyText}>작성한 리뷰가 아직 없어요.</Text>
            )}
          </View>
        )}
      />
...
function MyReviewCard({ review }: { review: ReviewItem }) {
  ...
  const handlePress = () => {
    navigation.navigate('BookReviewCreate', {
      book: {
        source: 'review',
        external_api_id: String(review.book),
